...
1. **`packages/tokens-core`** defines each token type as a `TokenTypeDefinition` (`configSchema`, `generate()`, `matchTrigger()`), registered in `registry.ts`. The 16 shipped types are grouped a few to a file: `network-tokens.ts`, `document-tokens.ts`, `pdf-clone-tokens.ts`, `cloud-tokens.ts`. `generate()` produces artifacts (pixel URLs, documents, DNS hostnames); artifacts are DB-backed (`token_files`, base64) — never filesystem paths.
...
4. **`packages/shared`** holds the zod schemas that are the single source of truth for API validation and MCP tool definitions — never redefine shapes locally. `apps/mcp` derives its token-type enum from `tokenTypeSchema`. A test in `packages/tokens-core` asserts that enum matches the registry, so a type registered in one place and not the other fails the build.
